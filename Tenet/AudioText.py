...
class Listner(BaseClass):
    ...
    def _audio_to_text(self, audio):
        ...
        try:
            ...
            _text = str(self.__recognizer.recognize_google(audio))
        except RequestError:
    ...
    def __callback(self, _recognizer, audio):                
        self._threads()
        self._write_to_file(self._audio_files_dir+self._file_middle+self._audio_file_extn,audio)
    ...
    def _listen_in_background(self):
        try:
            self._file_middle = str(datetime.now().strftime('%Y-%m-%d_%H_%M_%S'))
        ...
        self._write_to_file(self._text_files_dir+self._file_middle+self._text_file_extn, self._text, False,'w')
        self._polarity = self._sentimeter._score(self._text)
